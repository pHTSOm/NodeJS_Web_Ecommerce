...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    const { items, shipping, payment, discountCode, useLoyaltyPoints } =
      req.body;
...
async function fetchProductDetails(productId, variantId) {
  try {
    ...
    console.error(
      `Error fetching product #${productId} details:`,
      error.message
    );
    return null;
  }
  ...
  try {
    ...
    if (status === "delivered" && previousStatus !== "delivered") {
      ...
      (async () => {
        try {
          console.log(
            `Order #${orderId} delivered - updating product sales counts`
          );
          ...
          if (order.OrderItems && order.OrderItems.length > 0) {
            const updatePromises = order.OrderItems.map(async (item) => {
              ...
              while (retries < maxRetries) {
                try {
                  console.log(
                    `Updating salesCount for product #${item.productId} by ${item.quantity}`
                  );
                  ...
                  console.log(
                    `Updated salesCount for product #${item.productId}: Success`
                  );
                  ...
                  retries++;
                  console.error(
                    `Attempt ${retries} failed to update salesCount for product #${item.productId}:`,
                    error.message
                  );

                  if (retries >= maxRetries) {
                    console.error(
                      `Failed to update salesCount for product #${item.productId} after ${maxRetries} attempts`
                    );
                  } else {
                    // Exponential backoff: wait longer before each retry
                    const delay = Math.pow(2, retries) * 1000;
                    console.log(
                      `Retrying salesCount update for product #${item.productId} in ${delay}ms...`
                    );
            ...
            try {
              ...
              console.log(
                `Completed sales count updates for order #${orderId}`
              );
            } catch (batchError) {
              console.error(
                `Error processing batch sales count updates for order #${orderId}:`,
                batchError
              );
            }
          }
    ...
    try {
      const shouldNotify = ["confirmed", "shipped", "delivered"].includes(
        status
      );
...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    console.log(
      `Found ${orders.length} orders with status filter: ${status || "all"}`
    );
